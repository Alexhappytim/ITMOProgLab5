...
public class ProcessingThread extends Thread{
    ...
    public void run() {
        ...
        if (Server.dbConnection.checkUser(request.getLogin(), request.getPassword()) || request.getCommand().startsWith("register")) {
            ...
            Response resp = Server.commandManager.runCommand(request.getCommand(), request.getArg(), Server.dbConnection.getUserId(request.getLogin()));
            ...
            Response resp = new Response("Неправильный логин/пароль", Response.WRONGLOGIN);
            Server.lockCommands.unlock();
            executorService.submit(new ResponceThread(resp));
        }
    }
