package com.client.consoleManager.commandManager;


import com.client.Client;
import common.dragon.*;
import common.network.Response;
import com.client.consoleManager.commandManager.commands.withArg.*;
import com.client.consoleManager.commandManager.commands.withoutArg.*;
...
public class ClientCommandManager {
    ...
    public ClientCommandManager(){
        ...
        commandArgHashMap.put("login", new Login());
    }
    public Response runCommand(String command){
        String[] com = command.split(" ");
        if(commandHashMap.containsKey(com[0])){
                return commandHashMap.get(com[0]).execute();
        }else if(commandArgHashMap.containsKey(com[0])){
                return commandArgHashMap.get(com[0]).execute(command.replaceFirst(com[0] + " ", ""));
        }else{
            Client.consoleManager.println("Нет такой команды");
            return null;
        }
    }

    public Response runCommand(String command, Scanner scanner){
        String[] com = command.split(" ");
        if(commandHashMap.containsKey(com[0])){
                return commandHashMap.get(com[0]).executeFromScript(scanner);
        }else if(commandArgHashMap.containsKey(com[0])){
               return commandArgHashMap.get(com[0]).executeFromScript(command.replaceFirst(com[0]+" ",""),scanner);
        }else{
            Client.consoleManager.println("Нет такой команды");
            return null;
        }
